...
{
	print_debug("uf: fuzzer_ctrl_log: bRequestType: 0x%x, bRequest: 0x%x, wValue: 0x%x, wIndex: 0x%x, wLength: %d\n",
			...
			const struct usb_ctrlrequest *ctrl)
{
	...
	ret = fuzzer_event_queue_add(&dev->queue, USB_FUZZER_EVENT_CONTROL,
					sizeof(*ctrl), ctrl);
...
static int fuzzer_ioctl_ep0_read(struct fuzzer_dev *dev, unsigned long value)
{
	...
	print_debug("uf: fuzzer_ioctl_ep0_read: length: %u\n", length);
...
static int fuzzer_ioctl_ep0_write(struct fuzzer_dev *dev, unsigned long value)
{
	...
	print_debug("uf: fuzzer_ioctl_ep0_write: length: %u\n", io.length);
