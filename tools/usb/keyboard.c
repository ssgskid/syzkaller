...
void usb_fuzzer_ep0_read(int fd, struct usb_fuzzer_event *event) {
	int rv = ioctl(fd, USB_FUZZER_IOCTL_EP0_READ, event);
	if (rv != 0) {
...
void usb_fuzzer_ep0_write(int fd, struct usb_fuzzer_ep_io *response) {
	int rv = ioctl(fd, USB_FUZZER_IOCTL_EP0_WRITE, response);
	if (rv != 0) {
...
int keyboard_connect(int fd) {
	...
	while (!done) {
		...
		usb_fuzzer_ep0_read(fd, (struct usb_fuzzer_event *)&event);
...
void keyboard_loop(int fd, int ep) {
	...
	while (true) {
		...
		memcpy(&io.inner.data[1], "\x00\x00\x00\x00\x00\x00\x00\x00", 8);
