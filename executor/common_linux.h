...
static void patch_usb_descriptor(char* device, char* device_id)
{
	...
	debug("patching usb: dev: %p, iface: %p, id: %p\n", dev, iface, id);
...
static uintptr_t syz_usb_connect(uintptr_t a0, uintptr_t a1, uintptr_t a2, uintptr_t a3, uintptr_t a4)
{
	int64_t speed = a0;
	int64_t dev_len = a1;
	char* dev = (char*)a2;
	char* dev_id = (char*)a3;
	struct vusb_connect_descriptors* conn_descs = (struct vusb_connect_descriptors*)a4;

	debug("syz_usb_connect: dev: %p, dev_id: %p\n", dev, dev_id);
	if (!dev)
		return -1;
	...
	if (dev_id)
		patch_usb_descriptor(dev, dev_id);

	debug("syz_usb_connect: patched device data:\n");
	debug_dump_data(dev, dev_len);
...
static uintptr_t syz_usb_control_io(uintptr_t a0, uintptr_t a1, uintptr_t a2)
{
	int fd = a0;
...
static uintptr_t syz_usb_ep_write(uintptr_t a0, uintptr_t a1, uintptr_t a2, uintptr_t a3)
{
	int fd = a0;
	uint16 ep = a1;
	uint32 len = a2;
